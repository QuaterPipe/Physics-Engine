...
namespace physics
{
	...
	Line::Line(const Vector2& a, const Vector2& b) noexcept
	{
		this->a = a;
		this->b = b;
	...
	Vector2 Line::GetVectorAlongLine(const f64& distance, const bool& startFromA) const noexcept
	{
		Vector2 begin = startFromA ? this->b - this->a : this->a - this->b;
		begin.Normalize();
	...
	void Line::Rotate(const Vector2& pivot, const f64& angle) noexcept
	{
	...
	{
		return Equal(Distance(a, v) + Distance(v, b), Distance(a, b));
	}
}